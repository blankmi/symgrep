...
You have access to a local binary called `symgrep` which allows you to list and extract precise code symbols from source files using Tree-sitter.
...
1. **Discover**: If you don't know the exact symbol name, run `symgrep list -f <path>` first to discover what's defined.
2. **Extract**: Run `symgrep extract -f <path> -s <symbol_name> --format=json` to get the exact code and location.
...
```bash
symgrep list -f <file_or_dir_path>
symgrep list -f <file_or_dir_path> --format=json
symgrep list -f <file_or_dir_path> -t function
```

#### Extract specific symbol source
```bash
symgrep extract -f <file_or_dir_path> -s <symbol_name> --format=json
```
